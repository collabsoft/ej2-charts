...
function getPoint(x, y, xAxis, yAxis, isInverted, series) {
    ...
    let xvalue = (xAxis.valueType === 'Logarithmic') ? logBase(((x === 0 || x < 0) ? 1 : x), xAxis.logBase) : x;
    let yvalue = (yAxis.valueType === 'Logarithmic') ? logBase(((y === 0 || y < 0) ? 1 : y), yAxis.logBase) : y;
    xvalue = valueToCoefficient(xvalue, xAxis);
    yvalue = valueToCoefficient(yvalue, yAxis);
    let locationX = isInverted ? yvalue * (yLength) : xvalue * (xLength);
    let locationY = isInverted ? (1 - xvalue) * (xLength) : (1 - yvalue) * (yLength);
...
class Axis extends ChildProperty {
     ...
     * @private
     */
    triggerLabelRender(chart, tempInterval, text) {
        let argsData;
        argsData = {
     ...
     * @return {void}
     */
    findWrap(width, currentX, currentLabel) {
        let intervalLength = this.rect.width / this.visibleLabels.length;
        let textCollection = currentLabel.text.split(' ');
        let label = '';
        let labelCollection = [];
        let text;
        ...
        let height = (currentLabel.size.height * labelCollection.length);
...
/** @private */
class VisibleLabels {
    constructor(text, value, size = new Size(0, 0), index = 1) {
        this.text = text;
...
class CartesianAxisLayoutPanel {
    ...
    renderAxes() {
        ...
        let definitionElement = chart.renderer.createGroup({ id: chart.element.id + 'DefintionLine' });
    ...
    drawAxisLine(axis, index, plotX, plotY) {
        let chart = this.chart;
        let optionsLine = {};
        let rect = axis.rect;
        optionsLine = {
    ...
    drawYAxisLabels(axis, index) {
        ...
        let elementSize;
        let options;
        let padding = axis.majorTickLines.height + this.padding + axis.lineStyle.width / 2;
    ...
    drawYAxisTitle(axis, index) {
        let chart = this.chart;
        let labelRotation = (axis.opposedPosition) ? 90 : -90;
        let elementSize = measureText(axis.title, axis.titleStyle);
        let padding = axis.majorTickLines.height + axis.maxLabelSize.width + this.padding * 2;
        let rect = axis.rect;
        padding = axis.opposedPosition ? padding : -padding;
        ...
        let element = textElement(options, axis.titleStyle, axis.titleStyle.color, this.element);
    ...
    findLogNumeric(axis, logPosition, logInterval, value) {
        let range = axis.visibleRange;
        if (axis.valueType === 'Logarithmic') {
    ...
    drawXAxisLabels(axis, index) {
        ...
        let width = 0;
        let intervalLength = axis.rect.width / axis.visibleLabels.length;
        let label;
        for (let i = 0, len = axis.visibleLabels.length; i < len; i++) {
    ...
    drawXAxisTitle(axis, index) {
        let chart = this.chart;
        let elementSize = measureText(axis.title, axis.titleStyle);
        let padding = axis.majorTickLines.height + axis.maxLabelSize.height + this.padding * 2;
        let rect = axis.rect;
    ...
    renderTickLine(axis, index, majorTick, minorTick) {
        let options;
        let chart = this.chart;
    ...
    createZoomingLabel(chart, labelElement, axis, index) {
        let parentNode = document.getElementById(chart.element.id + 'AxisGroup' + index);
...
class Series extends SeriesBase {
     ...
     * @private
     */
    calculateStackedValue(isStacking100) {
 ...
 */
class MarkerExplode extends Data$1 {
    /**
    ...
    drawTrackBall(series, point, location, index) {
        ...
        let shape = series.type === 'Bubble' ? 'Circle' : marker.shape;
        let symbolId = this.elementId + '_Series_' + series.index + '_Point_' + point.index + '_Trackball' +
            (index ? index : '');
        let width = (series.type !== 'Bubble') ? marker.width : (point.regions[0].width);
        let height = (series.type !== 'Bubble') ? marker.height : (point.regions[0].height);
        let size = new Size(width + 5, height + 5);
        let border = (series.type === 'Bubble' || series.type === 'Scatter' ? series.border : marker.border);
        ...
        for (let i = 0; i < 2; i++) {
            let options = new PathOption(symbolId + '_' + i, i ? (marker.fill || point.color || (explodeSeries ? series.interior : '#ffffff')) : 'transparent', marker.border.width + (i ? 0 : 8), i ? borderColor : 'rgba(' + colorValue.r + ',' + colorValue.g + ',' + colorValue.b + ',0.2)', marker.opacity, null, null);
            let symbol = drawSymbol(location, shape, size, marker.imageUrl, options, '');
...
class Marker extends MarkerExplode {
    ...
    renderMarker(series, point, location, index) {
        ...
        let fill = marker.fill || (isBoxPlot ? series.interior : '#ffffff');
        let argsData;
...
class BaseLegend {
    ...
    renderText(chart, legendOption, group, textOptions, i) {
        ...
        let fontcolor = legendOption.visible ? legend.textStyle.color : hiddenColor;
            ...
            if (targetId.length === 2) {
                ...
                if (!isNaN(index)) {
                    ...
                    if (this.isChartControl) {
                        ...
                        let location = this.chart.removeSvgOffset(x, y);
                        showTooltip(this.chart.visibleSeries[0].points[index].x.toString(), location.x + 10, location.y + 10, element.offsetWidth, element.id + '_EJ2_Legend_Tooltip', getElement(this.chart.element.id + '_Secondary_Element'));
                    }
                }
            }
        }
    ...
    export(type, fileName) {
        ...
        image.onload = (() => {
            ...
            if (window.navigator.msSaveOrOpenBlob) {
                ...
                let imgURI = element.toDataURL('image/png').replace('image/png', 'image/octet-stream');
                createElement('a', {
                    attrs: {
...
let Chart = class Chart extends Component {
    ...
    renderTitle() {
        if (this.title) {
            let maxTitleSize = this.availableSize.width - this.margin.left - this.margin.right;
            let trimmedTitle = textTrim(maxTitleSize, this.title, this.titleStyle);
            this.elementSize = measureText(trimmedTitle, this.titleStyle);
            let options = new TextOption(this.element.id + '_ChartTitle', titlePositionX(this.availableSize, this.margin.left, this.margin.right, this.titleStyle, this.elementSize), this.margin.top + 3 * (this.elementSize.height / 4), 'start', trimmedTitle);
    ...
    renderBorder() {
        ...
        let rect = new RectOption(this.element.id + '_ChartBorder', this.background, this.border, 1, new Rect(width / 2, width / 2, this.availableSize.width - width, this.availableSize.height - width));
    ...
    renderAreaBorder() {
        if (this.chartAreaType === 'PolarRadar') {
            ...
            let rect = new RectOption(this.element.id + '_ChartAreaBorder', this.chartArea.background, this.chartArea.border, this.chartArea.opacity, this.chartAxisLayoutPanel.seriesClipRect);
            this.htmlObject = this.renderer.drawRectangle(rect);
            this.svgObject.appendChild(this.htmlObject);
        }
    }
    ...
    findAxisLabel(text) {
        let texts = ((text.replace(this.element.id, '')).replace('AxisLabel_', '')).split('_');
...
class Category extends Double {
    ...
    calculateVisibleLabels(axis) {
        /*! Generate axis labels */
        axis.visibleLabels = [];
        let tempInterval = axis.visibleRange.min - (axis.visibleRange.min % axis.visibleRange.interval);
        let position;
...
class StripLine {
    measureStripLine(axis, stripline, seriesClipRect) {
        let rect = this.getFromTovalue(stripline.start, stripline.end, stripline.size, stripline.startFromAxis, axis);
 ...
 */
class AreaSeries extends LineBase {
    /**
     * Render Area series.
     * @return {void}
     * @private
...
class StackingAreaSeries extends LineBase {
    ...
    render(series, xAxis, yAxis, isInverted) {
        ...
        let stackedvalue = series.stackedValues;
        let origin = series.chart.chartAreaType === 'PolarRadar' ?
 ...
 */
class SplineSeries extends LineBase {
    constructor() {
        super(...arguments);
        this.splinePoints = [];
    }
    ...
    findSplinePoint(series) {
        let spline = series.chart.splineSeriesModule;
        let value;
        let points = this.filterEmptyPoints(series);
...
class Crosshair {
    ...
    tooltipLocation(text, axis, bounds) {
        let padding = 5;
        let arrowPadding = 10;
        let tooltipRect;
 ...
 */
class Tooltip$1 extends Data$1 {
    /**
    ...
    renderPoint(point, isFirst, fill, firstText = true) {
        let argsData = {
    ...
    createTemplate(chart, data, parent, isFirst) {
        this.highlightPoints(data);
        ...
        if (!argsData.cancel) {
            ...
            let tooltipRect = this.seriesTooltipLocation(data, new ChartLocation(0, 0), new ChartLocation(0, 0));
    ...
    renderTooltip(chart, pointData, isFirst) {
        ...
        let x = 0;
        let y = 0;
        let location = pointData.series.type === 'BoxAndWhisker' ?
    ...
    seriesTooltipLocation(pointData, arrowLocation, tipLocation) {
        let series = pointData.series;
        let isBoxPlot = series.type === 'BoxAndWhisker';
        let symbolLocation = isBoxPlot ? this.getBoxLocation(pointData) : pointData.point.symbolLocations[0];
        ...
        let width = this.elementSize.width + (2 * this.padding);
        let height = this.elementSize.height + (2 * this.padding);
        let bounds = this.chart.chartAxisLayoutPanel.seriesClipRect;
        let markerHeight = 0;
    ...
    removeHighlight(chart, removeRect = false) {
        let item;
    ...
    getTooltipText(pointData, isFirst = true) {
        let series = pointData.series;
...
class Selection extends BaseSelection {
    ...
    drawDraggingRect(chart, dragRect) {
        ...
        if (element) {
            ...
            let dragGroup = this.renderer.createGroup({ id: this.draggedRectGroup });
    ...
    createCloseButton(x, y) {
        let closeIcon = this.renderer.createGroup({
            id: this.closeIconId,
    ...
    renderErrorBar(series) {
        ...
        for (let point of series.points) {
            if (point.visible && point.symbolLocations[0]) {
                ...
                let shapeOption = new PathOption(symbolId, '', errorbar.width, errorbar.color, null, '', errorDirection[0]);
                let path = this.chart.renderer.drawPath(shapeOption);
                series.errorBarElement.appendChild(path);
                let capOption = new PathOption(capId, '', errorBarCap.width, errorBarCap.color, null, '', errorDirection[1]);
 ...
 */
class AccumulationTooltipSettings extends ChildProperty {
}
__decorate$8([
    Property(false)
...
class AccumulationBase {
     ...
     * @private
     */
    getTooltipPoint(e, accumulation, x, y) {
        let target = e.target;
        target = accumulation.isTouch ? document.elementFromPoint(x, y) : target;
        let id = target.id.split(accumulation.element.id + '_Series_');
        if (id[1]) {
            let seriesIndex = parseInt(id[1].split('_Point_')[0], 10);
            let pointIndex = parseInt(id[1].split('_Point_')[1], 10);
            if (!isNullOrUndefined(seriesIndex) && !isNaN(seriesIndex) && !isNullOrUndefined(pointIndex) && !isNaN(pointIndex)) {
                let series = this.getSeriesFromIndex(seriesIndex, accumulation.visibleSeries);
                if (series.enableTooltip) {
...
let AccumulationChart = class AccumulationChart extends Component {
    ...
    setSecondaryElementPosition() {
        ...
        let targetElement = createElement('div', {
    ...
    renderTitle() {
        ...
        let maxTitleSize = this.availableSize.width - this.margin.left - this.margin.right;
        let trimmedTitle = textTrim(maxTitleSize, this.title, this.titleStyle);
        let elementSize = measureText(trimmedTitle, this.titleStyle);
...
class AccumulationDataLabel extends AccumulationBase {
     ...
     */
    move(e, x, y, isTouch) {
        let location = this.accumulation.removeSvgOffset(x, y);
...
class AccumulationTooltip {
     ...
     */
    renderTooltip(point, seriesIndex) {
        let element = getElement(this.targetId);
     ...
     */
    getTooltipContent(point, seriesIndex) {
        if (this.tooltipOption.template && this.templateFn) {
            let templates = this.templateFn(point);
            let element = createElement('div');
    ...
    tooltipCustomization(args) {
        ...
        let content = document.getElementsByClassName('e-tooltip-wrap')[0];
        let font = this.tooltipOption.textStyle;
        let position = this.tooltip.position.split(' ');
        let borderColor = this.tooltipOption.border.color || this.currentPoint.color;
        let pointerSize = 8;
        let outerWidth;
        let innerWidth;
        ...
        let borderWidth = this.tooltipOption.border.width;
     ...
     * @private
     */
    updatePosition(id, x, y, pointId) {
        let pointElement = getElement(pointId);
        let translate = pointElement.getAttribute('transform');
        if (!isNullOrUndefined(translate) && translate !== '') {
            translate = translate.replace('translate(', '');
            translate = translate.replace(')', '');
            let tx = parseInt(translate.split(',')[0], 10);
            let ty = parseInt(translate.split(',')[1], 10);
            x = !isNaN(tx) ? tx + x : x;
            y = !isNaN(ty) ? ty + y : y;
        }
        let tooltip = getElement(id);
     ...
     */
    getTooltipText(point, tooltip, seriesIndex) {
        let format = tooltip.format ? tooltip.format : '${point.x} : ${point.y}';
        let series = getSeriesFromIndex(seriesIndex, this.accumulation.visibleSeries);
        return this.parseTemplate(point, format, series);
    }
    /**
     * To parse the tooltip template
     */
    parseTemplate(point, format, series) {
        let value;
        let textValue;
        for (let dataValue of Object.keys(point)) {
